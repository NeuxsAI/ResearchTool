...
export function PDFViewer({ url, onSelection, annotations = [], onAnnotationClick }: PDFViewerProps) {
  ...
  const [currentSelection, setCurrentSelection] = useState<{ position: any; content: any } | null>(null);
  ...
  return (
    <div className="absolute inset-0">
      <PdfLoader url={proxyUrl} beforeLoad={<LoadingOverlay message="Loading PDF..." />}>
        {(pdfDocument) => (
          <PdfHighlighter
            ...
            onSelectionFinished={(position, content, hideTipAndSelection) => {
              ...
              selectionRef.current = { position, content, hideTip: hideTipAndSelection };
              ...
              return (
                <div className="flex gap-1 p-2 bg-slate-800/90 rounded-md shadow-lg border border-[#2a2a2a]">
                  <button
                    ...
                    className="px-2 py-1 text-xs text-white bg-violet-600 opacity-80 hover:opacity-100 rounded"
                  >
                    Add Highlight
                  </button>
                  <button
